...
function AssignmentField({
  ...
  const selectedIds = selectedAssignmentIds(assignment.assignedTo)
  const selectedMembers = assignment.members.filter(member =>
  ...
  return (
    <div className="block text-xs font-semibold text-muted">
      ...
      <details className="group rounded-lg border border-line bg-white">
        <summary className="flex min-h-[46px] cursor-pointer list-none items-center justify-between gap-3 px-3 py-2 text-sm text-ink outline-none transition focus-visible:ring-4 focus-visible:ring-sage/15 group-open:border-b group-open:border-line [&::-webkit-details-marker]:hidden">
          <span className="min-w-0 flex-1">
            {selectedMembers.length === 0 ? (
              <span className="text-muted">Choose assignees</span>
            ) : (
              <span className="flex flex-wrap gap-1.5">
                {selectedMembers.slice(0, 3).map(member => (
                  <span
                    key={member.userId}
                    className="inline-flex max-w-full items-center gap-1.5 rounded-full bg-[var(--ws-accent-soft,#e9f0ec)] py-1 pl-1 pr-2 text-[10px] font-bold text-[var(--ws-accent,#375b4b)]"
                  >
                    <Avatar person={member} className="h-5 w-5 text-[8px]" />
                ...
                {selectedMembers.length > 3 && (
                  <span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-bold text-muted">
                    +{selectedMembers.length - 3}
                  </span>
                )}
              </span>
        ...
        <div className="grid gap-1 p-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
          {assignment.members.map(member => {
    ...
    <form onSubmit={onSubmit} className="flex min-h-0 flex-1 flex-col">
      <div className="min-h-0 flex-1 overflow-y-auto pr-1">
        <div
          className={
            assignment
              ? 'grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.85fr)]'
              : 'space-y-4'
          }
        >
          <div className="space-y-4">
            ...
            <label className="block text-xs font-semibold text-muted">
              ...
              <textarea
                ...
                className="mt-2 w-full resize-y rounded-lg border border-line bg-white px-3 py-2.5 text-sm text-ink outline-none transition placeholder:text-muted/60 focus:border-sage focus:ring-4 focus:ring-sage/15"
              />
            </label>
            {ideas && (
              <IdeaSelect
          ...
          <div className="space-y-4">
            {assignment && <AssignmentField assignment={assignment} />}
            <label className="block text-xs font-semibold text-muted">
              {targetLabel}{' '}
              <span className="font-normal text-muted/80">(optional)</span>
            ...
            {values.trackGoal && (
              <div className="grid gap-3 sm:grid-cols-[1fr_100px] lg:grid-cols-1 xl:grid-cols-[1fr_100px]">
                <label className="block text-xs font-semibold text-muted">
                  Goal name
                  <input
                    value={values.goal}
