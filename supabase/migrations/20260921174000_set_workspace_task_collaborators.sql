-- Collaborative assignment: let checked RPCs manage task_collaborators while
-- keeping the table itself read-only to authenticated clients.
...
begin
  ...
  where not exists (
    ...
    where tc.task_id = p_task_id
      and tc.user_id = selected.user_id
      and tc.removed_at is null
  );
  ...
  set removed_at = null,
      removed_by = null
  where tc.task_id = p_task_id
    and tc.user_id = any(v_user_ids)
    and tc.removed_at is not null
    and not exists (
      select 1
      from public.task_collaborators active
      where active.task_id = tc.task_id
        and active.user_id = tc.user_id
